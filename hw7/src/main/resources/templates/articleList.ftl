    ...
    <div class="col s12 m6 l4">
        <div class="card large hoverable">
            ...
            <div class="card-action">
                ...
                <a>
                    <i class="material-icons small favorite-star right" data-articleid="${a.id}">${ (preferenceMap?api.containsKey(a.id) && preferenceMap?api.get(a.id).favorite)?then("star","star_border") }</i>
                </a>
                <a>
                    <i class="material-icons small read right" data-articleid="${a.id}">${ (preferenceMap?api.containsKey(a.id) && preferenceMap?api.get(a.id).read)?then("drafts","markunread") }</i>
                </a>
            </div>
        </div>
    </div>
